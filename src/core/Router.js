...
class Router {
  ...
  async preloadAllComponents() {
    ...
    const loadPromises = Array.from(this.routes.entries()).map(
  ...
  async loadDynamicComponent(componentPath, cacheKey) {
    ...
    try {
      ...
      console.error(
        `❌ Failed to load dynamic component: ${componentPath}`,
        error,
      );
      return null;
    }
  ...
  async render() {
    ...
    if (!middlewareResult.success) {
      console.log(
        `🚫 Middleware blocked access to ${path}: ${middlewareResult.reason}`,
      );
    ...
    if (!ComponentClass) {
      ...
      if (dynamicMatch) {
        ...
        ComponentClass = await this.loadDynamicComponent(
          dynamicMatch.componentPath,
          cacheKey,
        );
      }
    }
    ...
    try {
      ...
      if (pathSegments.length > 0) {
        ...
        try {
          ...
          customLayoutTagName = `app-${CustomLayoutClass.name
            .replace(/([A-Z])/g, "-$1")
            .toLowerCase()
            .replace(/^-/, "")}`;
        } catch (e) {
      ...
      if (customLayoutTagName && CustomLayoutClass) {
        ...
        setTimeout(() => {
          ...
          const customLayoutElement =
            this.outlet.querySelector(customLayoutTagName);
