...
class Dropdown extends HTMLElement {
      ...
      case "value":
        if (newValue) {
          ...
          if (this.hasAttribute("multiple")) {
            newValue
              .split(",")
              .forEach((v) => this.selectedValues.add(v.trim()));
          } else {
  ...
  updateSelection() {
    ...
    if (this.hasAttribute("multiple")) {
      // Multi-select mode
      const tags = Array.from(this.selectedValues)
        .map((value) => {
      ...
      this.selection
        .querySelectorAll(".upo-dropdown-tag-remove")
        .forEach((removeBtn) => {
  ...
  updateOptions() {
    ...
    this.filteredOptions = options.filter((option) => {
      if (this.searchTerm) {
        return (
          option.textContent.toLowerCase().includes(this.searchTerm) ||
    ...
    const optionsHtml = this.filteredOptions
      .map((option, index) => {
    ...
    this.optionsContainer
      .querySelectorAll(".upo-dropdown-option")
      .forEach((optionEl) => {
