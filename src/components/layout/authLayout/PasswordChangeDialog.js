...
class PasswordChangeDialog extends HTMLElement {
  ...
  ensureInputInitialization() {
    ...
    inputs.forEach((input) => {
      if (
        input.type === "password" &&
        !input.querySelector(".toggle-password")
      ) {
  ...
  async handleSubmit() {
    ...
    try {
      ...
      const response = await fetch(
        `/api/users/${userData.id}/change-password`,
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        Toast.show({
          title: "Success",
          message:
            "Password changed successfully. You will be logged out to login with your new password.",
          variant: "success",
          duration: 3000,
        });
