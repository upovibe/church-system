...
class PageUpdateModal extends HTMLElement {
  ...
  setPageData(pageData) {
    ...
    setTimeout(() => {
      const bannerFileUpload = this.querySelector(
        'ui-file-upload[data-field="banner"]',
      );
  ...
  async updatePage() {
    try {
      ...
      const contentWysiwyg = this.querySelector(
        'ui-wysiwyg[data-field="content"]',
      );
      const categoryDropdown = this.querySelector(
        'ui-dropdown[data-field="category"]',
      );
      const statusRadioGroup = this.querySelector(
        'ui-radio-group[name="status"]',
      );
      const metaDescriptionTextarea = this.querySelector(
        'ui-textarea[name="meta-description"]',
      );
      ...
      const pageData = {
        ...
        meta_description: metaDescriptionTextarea
          ? metaDescriptionTextarea.value
          : "",
        meta_keywords: metaKeywordsInput ? metaKeywordsInput.value : "",
        is_active: statusRadioGroup
          ? statusRadioGroup.value === "active"
            ? 1
            : 0
          : 0,
      ...
      const response = await api
        .withToken(token)
        .put(`/pages/${this.pageData.id}`, formData);
      ...
      const updatedPage = {
        ...
        ...pageData,
        banner_image:
          response.data.data?.banner_image || this.pageData.banner_image,
