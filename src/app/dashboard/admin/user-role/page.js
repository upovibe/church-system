...
class UserRolePage extends App {
  ...
  connectedCallback() {
    ...
    this.addEventListener("user-role-deleted", (event) => {
      ...
      const updatedUserRoles = currentUserRoles.filter(
        (role) => role.id !== deletedUserRoleId,
      );
  ...
  async loadData() {
    try {
      ...
      Toast.show({
        title: "Error",
        message:
          error.response?.data?.message || "Failed to load user roles data",
        variant: "error",
        duration: 3000,
      });
    }
  ...
  onView(event) {
    const { detail } = event;
    const viewUserRole = this.get("userRoles").find(
      (role) => role.id === detail.row.id,
    );
    if (viewUserRole) {
      this.closeAllModals();
  ...
  onEdit(event) {
    const { detail } = event;
    const editUserRole = this.get("userRoles").find(
      (role) => role.id === detail.row.id,
    );
    if (editUserRole) {
      this.closeAllModals();
  ...
  onDelete(event) {
    const { detail } = event;
    const deleteUserRole = this.get("userRoles").find(
      (role) => role.id === detail.row.id,
    );
    if (deleteUserRole) {
      this.closeAllModals();
