...
class UsersPage extends App {
    ...
    getHeaderCounts() {
        ...
        users.forEach((user) => {
            const isActive = String(user.status || '').toLowerCase() === 'active' || Number(user.is_active) === 1;
            if (isActive) active += 1; else inactive += 1;
            if (user.role) roleSet.add(String(user.role));
        });
                ...
                <div class="bg-gradient-to-r from-blue-600 to-purple-600 rounded-xl shadow-lg p-5 text-white">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6">
                        <div>
                            ...
                            <p class="text-blue-100 text-base sm:text-lg">Manage user accounts and permissions</p>
                        </div>
                        ...
                        <div class="bg-white bg-opacity-20 backdrop-blur-sm rounded-lg p-4 sm:p-6 border border-white border-opacity-20">
                            <div class="flex items-center">
                                <div class="size-10 flex items-center justify-center bg-blue-500 rounded-lg mr-3 sm:mr-4 flex-shrink-0">
                                    <i class="fas fa-user-tag text-white text-lg sm:text-xl"></i>
                                </div>
                                <div class="min-w-0 flex-1">
                                    <div class="text-xl sm:text-2xl font-bold">${c.roles}</div>
                                    <div class="text-blue-100 text-xs sm:text-sm">Roles</div>
                                </div>
                            </div>
                        </div>
                        <div class="bg-white bg-opacity-20 backdrop-blur-sm rounded-lg p-4 sm:p-6 border border-white border-opacity-20">
