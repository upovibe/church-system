...
class SystemSettingsPage extends App {
  ...
  connectedCallback() {
    ...
    this.addEventListener("setting-deleted", (event) => {
      ...
      const updatedSettings = currentSettings.filter(
        (setting) => setting.id !== deletedSettingId,
      );
  ...
  async loadData() {
    try {
      ...
      Toast.show({
        title: "Error",
        message:
          error.response?.data?.message || "Failed to load settings data",
        variant: "error",
        duration: 3000,
      });
    }
  ...
  onView(event) {
    const { detail } = event;
    const viewSetting = this.get("settings").find(
      (s) => s.id === detail.row.id,
    );
    if (viewSetting) {
      this.closeAllModals();
  ...
  onEdit(event) {
    const { detail } = event;
    const editSetting = this.get("settings").find(
      (s) => s.id === detail.row.id,
    );
    if (editSetting) {
      this.closeAllModals();
  ...
  onDelete(event) {
    const { detail } = event;
    const deleteSetting = this.get("settings").find(
      (s) => s.id === detail.row.id,
    );
    if (deleteSetting) {
      this.closeAllModals();
