...
function uploadGiveBanner($file) {
    $uploadDir = __DIR__ . '/../uploads/give/';
    ...
    if (extension_loaded('gd')) {
        $thumbnails = generateGiveThumbnails($originalPath, $filename, $thumbnailsDir);
    } else {
        error_log('GD extension not available. Skipping thumbnail generation.');
    }
    
    return [
        'original' => 'uploads/give/' . $filename,
...
function deleteGiveBanner($imagePath) {
    ...
    $thumbnailsDir = $baseDir . 'uploads/give/thumbnails/';
...
/**
 * Generate thumbnails for give banner
 * 
 ...
 */
function generateGiveThumbnails($originalPath, $filename, $thumbnailsDir) {
    ...
    foreach ($sizes as $size => $dimensions) {
        ...
        if (createThumbnail($originalPath, $thumbnailPath, $dimensions[0], $dimensions[1])) {
            $thumbnails[$size] = 'uploads/give/thumbnails/' . $thumbnailFilename;
        }
    }
